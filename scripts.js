document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadCSV() {
        try {
            ...
            if (columnIndexA === -1 || columnIndexB === -1 || statusColumnIndex === -1) {
            ...
            if (dataA.length > 0 && dataB.length > 0 && statusData.length > 0) {
                showRow(currentRow);
            } else {
    ...
    function showRow(index) {
        if (dataA.length === 0 || dataB.length === 0 || statusData.length === 0) return;
